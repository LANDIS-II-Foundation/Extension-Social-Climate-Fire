...
This is only to serve as a educational tool  
...
Here we have feild data for three species, across 100 sites. This includes the estimated Age, DBH and whether the tree was dead. We have included here an emperical constant for the relationship between bark thickness and DBH (BT_coeff). This is used to calculate the column Bark. 
...
### Raster data
We also have (example) rasters for the area of these sites. These include the previous years climatic water deficit, the effective wind speed on the day of,estimations of fuels, and the post fire RDNBR. These are constructed into a dataframe and joined with the plot level data by site name. Note: Here every site represents a pixel in the feild, this will have to be adjusted to fit data coverage. 
...
Here we fit the site level mortality, for this study we are using RDNBR as the measure of that. 
...
It seems there is no relationship with climatic water deficit.We can refit the model without it. 
...
* SiteMortalityB3  15.047521  << The parameter fit for site level Effective Windspeed. 
