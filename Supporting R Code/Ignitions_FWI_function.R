...
##Extract all fire Julian days
#fire.days <- as.data.frame(cbind(fire.dat$FIRE_YEAR, fire.dat$DISCOVERY1)) #Extracting year and julian day
#colnames(fire.days) <- c("YEAR", "J_DAY")
#fire.days.sort <- fire.days[order(fire.days[,1]),] #sorting by year
...
igns.list <- list()
for (i in 1:length(ign.types[1:2])){ ##RIGHT NOW THIS DOESN'T INCLUDE RX BURNS - WAITING ON NEW CLIMATE DATA
  ...
  fire.days.short <- subset(fire.days.count, fire.days.count$YEAR < 2000) ##restricting fire records to climate file years
...
##Fitting regression for number of fires ~ FWI
##Trying a 0-inflated Poisson distribution
zeroinf.mod <- zeroinfl(igns.list[[1]][,2]~igns.list[[1]][,1], dist="poisson", data=igns.list[[2]])
summary(zeroinf.mod)

#Evaluating the coefficients
e <- exp(1) #for creating e
test <- e^(-3.33502 + (0.04807*igns.list[[2]][,1])) ##Testing the coefficients produced by the zero-inflated model
plot(test, ylab = "number of fires", xlab = "day")
